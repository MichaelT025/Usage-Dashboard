...
describe('aggregate', () => {
  it('isolates provider failures so one thrown adapter does not affect others', async () => {
    const adapters: IProviderAdapter[] = [
      ...
      adapter('codex', 'Codex', async () => {
        throw new Error('failed with Bearer sk-ant-explosion');
      }),
      adapter('opencode-go', 'OpenCode Go', async () =>
        okUsage('opencode-go', 'OpenCode Go'),
      ),
    ];
    ...
    expect(result.providers[1]?.error?.message).not.toContain(
      'sk-ant-explosion',
    );
